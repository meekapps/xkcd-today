...
@property (strong, nonatomic) PersistenceManager *persistenceManager;
@end
...
+ (instancetype)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        instance = [[XKCD alloc] initWithPersistenceManager:PersistenceManager.sharedManager];
    });
    return instance;
}
...
- (void) toggleFavorite:(NSNumber *)index {
    XKCDComic *comic = [self fetchComicWithIndex:index];
...
- (NSArray<XKCDComic*>*) fetchAllDownloaded {
    ...
    NSManagedObjectContext *context = self.persistenceManager.managedObjectContext;
    NSError *error = nil;
...
- (XKCDComic*) fetchComicWithIndex:(NSNumber*)index {
    NSManagedObjectContext *managedObjectContext = self.persistenceManager.managedObjectContext;
